...
void SD_EraseBlocks(unsigned long firstBlock, unsigned long lastBlock){
    ...
    SD_Command(CMD32, firstBlock);


    SD_Command(CMD33, lastBlock);
...
void initSD(void){
    const unsigned char last_OSCCON = OSCCON;
...
# 396 "SD_PIC.c"
    ...
    if(SDCard.SDversion == 2){
        ...
        SDCard.size = tempSize * 0.524288;
        SDCard.numBlocks = (unsigned long)(SDCard.size * 2048);
     }
     else{
    ...
    while(!OSCCONbits.IOFS){ _delay((unsigned long)((20)*(32000000/4000000.0))); }
...
void multiple_block_write(void)
{
    ...
    printf("MBW Start...\r\n");
    SD_MBW_Start(firstBlock, numWrites);
