...
static __attribute__((inline)) void Timer0_Mode_Selection(const timer0_t *_timer);





static __attribute__((inline)) void Timer0_Reigster_Size_Config(const timer0_t *_timer);
# 53 "hal_timer0.c"
uint8_t Timer0_Init(const timer0_t *_timer)
{
    uint8_t ret = 0;
...
uint8_t Timer0_Read_Value(const timer0_t *_timer, uint16_t *_value)
{
    ...
    else
    {
        l_tmr0l = TMR0L;
        l_tmr0h = TMR0H;
        *_value = (uint16_t)((l_tmr0h << 8) + l_tmr0l);
        ret = 1;
    }
    return ret;
}
...
static __attribute__((inline)) void Timer0_Prescaler_Config(const timer0_t *_timer)
{
    if(1 == _timer->prescaler_status_cfg)
    {
        (T0CONbits.PSA = 0);
        T0CONbits.T0PS = _timer->prescaler_value;
    }
    else if(0 == _timer->prescaler_status_cfg)
    {
       (T0CONbits.PSA = 1);
    }
    else{ }
